...
@Component({
  selector: 'ep-report-month-picker',
  templateUrl: './ep-report-month-picker.component.html',
  styleUrls: ['./ep-report-month-picker.component.css'],
  providers: [{ provide: DL_DATE_TIME_DISPLAY_FORMAT, useValue: "YYYY/MM"}]
})
export class EpReportMonthPickerComponent implements OnInit, AfterViewInit {
  ...
  dateSelected(event: any) {
    if (this._isPickerOpen && event.value) {
      $(this.currentElement).find('.month-picker-dropdown').dropdown('toggle');
    }
    if(event.value){
