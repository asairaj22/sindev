...
@Component({
  ...
  styleUrls: ['./document-landing.component.css', './../document-management.module.css'],
  encapsulation: ViewEncapsulation.None
})
export class DocumentLandingComponent implements OnInit, AfterViewInit {
  ...
  initDocumentLandingDetails(){
    ...
    this.apiService.invokePlatformServiceApi('/dmsLanding/virtualDirectories/init', 'POST', {}).subscribe(
      (res: any) => {
  ...
  onLandingTablePageChange(pageObj: any){
    var startIndex = pageObj.startIndex;
    var endIndex = pageObj.endIndex + 1;
    var itemsPerPage = this.itemsPerPage;
    var totalRecordObject = this.totalRecordObject;
    var lazyLoadOffsetCount = (pageObj.currentPage - 1) * itemsPerPage + 1;
    var totalRecordCount = pageObj.totalItems;
    if(Object.keys(totalRecordObject).length > 0){
      try{
        var i=0;
        var recordList = [];
        while(i < itemsPerPage){
          var currentObjectCount = lazyLoadOffsetCount + i;
          if(totalRecordObject[currentObjectCount]){
            recordList.push(totalRecordObject[currentObjectCount]);
            i++;
          }else if(currentObjectCount > totalRecordCount){
            break;
          }else{
              throw new Error('Record count exceeded');
          }
        }
        this.dataGridComponent.gridApi.setRowData(recordList);
      }catch(error){
        this.loadDocuments(this.SELECTED_FOLDER_FOLDERID, startIndex, 100, true);
      }
    }
    document.getElementById("landingSearchOutput").innerHTML = ("Showing " + ( (Math.sign(startIndex) == -1 ? 0 : (startIndex + 1)) ) + " - " + endIndex + " of " + totalRecordCount);
  };

  invokeChangePageSize(pageSize: any){
    this.paginationComponent.pageSize = this.itemsPerPage = pageSize;
    this.paginationComponent.setPage(1);
  };
  
  loadDocuments(folderId: any, from: any, size: any, isCallFromPageChange?: boolean){
    ...
    this.documentManagementService.getFolderConent({folderId, from, size}, requestBody, this.getAdditionalHeaders(), 
      (res: any) => {
        ...
        // Implemented below lines for lazy loading
        // this.totalRecordObject = this.totalRecordObject || {};
        // var lazyLoadOffsetCount = from + 1;
        // (folderContentList || []).forEach(function(record: any, recordIndex: number){
        //   _this.totalRecordObject[lazyLoadOffsetCount + recordIndex] = record;
        // });
        // if(isCallFromPageChange){
        //   let curPageRecords = this.getCurrentPageRecords(this.totalRecordObject, lazyLoadOffsetCount, this.itemsPerPage);
        //   this.dataGridComponent.gridApi.setRowData(curPageRecords);
        // }else{
        //   this.paginationComponent.totalRecords = this.landingTableTotalRecords = responseBody.recordsTotal;
        //   this.paginationComponent.setPage(1);
        // }
  ...
  getCurrentPageRecords(totalRecordObject: any, startIndex: number, limit: number){
    var recordList = [];
    var i=0;
    while(i < limit){
      var currentObjectCount = startIndex + i;
      if(totalRecordObject[currentObjectCount]){
        recordList.push(totalRecordObject[currentObjectCount]);
        i++;
      }else{
  ...
  createFolder(){
    ...
    this.apiService.invokePlatformServiceApi('/dmsLanding/virtualDirectories/' + this.ADD_FOLDER['parent']  + '/' + this.ADD_FOLDER['child'], 'POST', addFolderReq, this.getAdditionalHeaders()).subscribe(
      res => {
        this.apiService.loaderHide('loader');
        var responseBody = (res.body || {});
  ...
  updateFolderPrivilegeToSession(){
    this.apiService.invokePlatformServiceApi('/dmsLanding/folderPrivilege', 'GET').subscribe(
      (res: any) => {
  ...
  setCheckinMimeType(){
    ...
    this.checkinFileName = file.name;
  ...
  checkInDoc(){
    ...
    this.SELECTED_DOCUMENT.fileName = this.checkinFileName;
    var filesName = file.name.split(".");
  ...
  getDocs(type: any, value: any){
    ...
    this.apiService.invokePlatformServiceApi('/dmsLanding/documentList/' + this.GET_DOCUMENTLIST_BYTAGORTYPE['criteria'] + '/' + this.GET_DOCUMENTLIST_BYTAGORTYPE['value'], 'GET').subscribe(
      (res: any) => {
        this.apiService.loaderHide('loader');
        if(res.body){
        ...
        this.loadDocuments(this.dms.currentFolder, 0, 10000); 
      },
      (err: any) => {
        this.apiService.loaderHide('loader');
        this.toastr.error((this.apiService.fetchErrorMessage(err) || 'Something went wrong. Please check console'), 'FAILED');
  ...
  updateDocument(){
    ...
    this.apiService.invokePlatformServiceApi('/dmsLanding/udpateDocument', 'POST', JSON.parse(JSON.stringify(this.SELECTED_DOC)), this.getAdditionalHeaders()).subscribe(
      (res: any) => {
        this.apiService.loaderHide('loader');
  ...
  invokeSearchOverGrid(searchObj: object){
    //var regex = new RegExp("[<>()$#@!~%?+=;]+$");
    var regex = new RegExp("[<>()$#@!~%?+=;*{}\\\\^|:\\\\/]");
    this.searchedDataDT = searchObj['searchedDataDT'];
    this.searchedDataScope = searchObj['searchedDataScope'];
  ...
  getDocHistory(rowData: any){
    ...
    this.apiService.invokePlatformServiceApi('/dmsLanding/viewHistory/' + name + '/' + id + '/' + folderId, 'GET', null, this.getAdditionalHeaders()).subscribe(
      (res: any) => {
  ...
  submitApprove(){
    ...
    this.apiService.invokePlatformServiceApi('/dmsLanding/approveDocument', 'POST', reqObj, this.getAdditionalHeaders()).subscribe(
      (res: any) => {
        this.apiService.loaderHide('loader');
        if((res.body || {}).STATUS == true){
  ...
  cancelCheckOut(){
    ...
    this.apiService.invokePlatformServiceApi('/dmsLanding/cancelCheckout', 'POST', reqObj, this.getAdditionalHeaders()).subscribe(
      (res: any) => {
        this.apiService.loaderHide('loader');
        if((res.body || {}).STATUS == true){
  ...
  submitReject(){
    ...
    let reqObj = JSON.stringify(this.SELECTED_DOCUMENT);
    this.apiService.invokePlatformServiceApi('/dmsLanding/rejectDocument', 'POST', reqObj, this.getAdditionalHeaders()).subscribe(
      (res: any) => {
        this.apiService.loaderHide('loader');
        if((res.body || {}).STATUS == true){
  ...
  submitComfirmRemove(){
    ...
    let reqObj = JSON.stringify(this.SELECTED_DOCUMENT);
    this.apiService.invokePlatformServiceApi('/dmsLanding/removeDocument', 'POST', reqObj, this.getAdditionalHeaders()).subscribe(
      (res: any) => {
        this.apiService.loaderHide('loader');
        if((res.body || {}).STATUS == true){
  ...
  deleteFolderConfirmed(){
    ...
    this.apiService.invokePlatformServiceApi('/dmsLanding/virtualDirectories/' + reqObj.folderId + '/' + reqObj.folderName + '/' + reqObj.folderLeft + '/' + reqObj.folderRight, 'POST', reqObj, this.getAdditionalHeaders()).subscribe(
      (res: any) => {
        this.apiService.loaderHide('loader');
  ...
  splitDragEnd(params: any){
    this.dataGridComponent.gridApi.sizeColumnsToFit();
  }
  ...
  {
    if(this.dataGridComponent && this.dataGridComponent.gridApi){
